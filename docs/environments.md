...
**Using Impasse from Cursor.** The review path runs unmodified, but a Cursor session is
`undetermined` by default and *only* an operator assertion can change that — see the "Cursor host
