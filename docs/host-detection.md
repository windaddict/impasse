...
| `composer` | *(no marker — assertion only)* | **asserted** when `IMPASSE_HOST=composer`; never auto-detected | Cursor's own model family (Anysphere). Distinct from the `cursor` row below, which is the **Auto router** and is not a lab. Base-model provenance is not fully public — see SKILL.md's Composer caveat | n/a — operator assertion |
